...
- Figure : Ovaskainen fourth corner without Phylogenie 
...
```{r comparison-time-accuracy, echo=FALSE, include=TRUE, fig.show = "hold",fig.pos = "H", fig.align='center'}
...
sp_pictures <- c("des.jpg",
                 ...
                 "alpine_plants_.jpg",
                 "oribatid_mites_.png")
result[23, 2:8] <- sprintf('\\includegraphics[height=1.4cm, width=1.4cm]{%s}', sp_pictures)

kable(result, booktabs=TRUE, "latex", align=c('l', rep('r',7)), escape=FALSE) %>%
      row_spec(0, bold=TRUE, align='c') %>%
      column_spec(1, bold=TRUE, width="3.5cm") %>%
      #column_spec(2:8, width="1.5cm")  %>%
      group_rows("T.sample (secondes)", 5, 7, escape=FALSE) %>%
      group_rows("Species intercept: $\\beta_0$", 8, 10, escape=FALSE) %>%
      group_rows("Species effects: $\\beta$", 11, 13, escape=FALSE) %>%
      group_rows("Factor loadings: $\\lambda$", 14, 16, escape=FALSE) %>%
      group_rows("Latent variables: $W$", 17, 19, escape=FALSE) %>%
...
```{r comparison-sampler-efficiency, echo=FALSE, include=TRUE, fig.show = "hold",fig.pos = "H", fig.align='center'}
...
load("~/Documents/projet_BioSceneMada/Report/Internship Report/data/mada_mod.RData")
